...
int main()
{
    ...
    if ( (out2=fopen("zcor_out.txt","a")) == NULL) {
	fprintf(stderr, "Error opening zcor_out.txt\n");
	exit(1);
    }
...
/*Added to write station specific zcor, vr and global vr file*/
for (i=0; i < nsta; i++)
    {
    fprintf(out2,"%d %.2f ",ss[i].zz,ss[i].vr);
    }
    fprintf(out2,"%.2f\n",VR);
