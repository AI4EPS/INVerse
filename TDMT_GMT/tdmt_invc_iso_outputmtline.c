...
FILE *fopen(), *par,*fd, *fd1, *fd2, *fd3, *out, *out2;


int main()
{
    int i,j,l,N,Np,Z,k,ntr,nsta,nn,*npts = NULL ,wflag=0;
    ...
    /**************/
    float fullMo, Mo, isoMo, Mw, Strike, Rake, Dip, Pdc, Piso, Pclvd,F;
    float St2, Rk2, Dp2;
    int isoflag;
    ...
    if ( (out2=fopen("mt_line.out","a")) == NULL) {
	fprintf(stderr, "Error opening mt_line.out  %s\n", MT_OUTFILE);
	exit(1);
    }
    ...
    tmp=(float *)malloc(sizeof(float)*10*10000);
...
/* moved down to also plot VR
    fprintf(out2,"%.3f  %.3f  %.3f  %.3f  %.3f  %.3f\n",
	    d_mt[0][0]/gfscale,d_mt[0][1]/gfscale,d_mt[0][2]/gfscale,d_mt[1][1]/gfscale,
	    d_mt[1][2]/gfscale,d_mt[2][2]/gfscale);
*/
...
/*Code to Convert Percentages to those based on Bowers moment*/
/*Fortran subroutines compute Dziewonski moment + isotropic*/
Piso=fabs(isoMo)/fullMo*100;
F=-eigen1/eigen3;
Pdc=(fabs(eigen3)/fullMo)*fabs(1-2*F)*100;
Pclvd=(fabs(eigen3)/fullMo)*fabs(F)*200;
/*End Percent Decomposition Modification 01122016*/

#ifndef ONLY_ERRORS_TO_STDERR
    fprintf(stderr,"isoMo: %g\n",isoMo);
    fprintf(stderr,"Mo=%g  (%g)\nMw=%.2f\nStrike=%.0f ; %.0f\nRake=%.0f ; %.0f\nDip=%.0f; %.0f\nPdc=%.0f\nPclvd=%.0f\nPiso=%.0f\n",
    ...
    fprintf(out2,"%.2f %.3f  %.3f  %.3f  %.3f  %.3f  %.3f\n",
            VR,d_mt[0][0]/gfscale,d_mt[0][1]/gfscale,d_mt[0][2]/gfscale,d_mt[1][1]/gfscale,
            d_mt[1][2]/gfscale,d_mt[2][2]/gfscale);
