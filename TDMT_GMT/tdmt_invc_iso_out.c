...
FILE *fopen(), *par,*fd, *fd1, *fd2, *fd3, *out, *outd, *outw, *outG;
...
int main()
{
  ...
  if(isoflag == 6)
   {
   fprintf(stderr,"Opening d, G and W outfiles\n");
   outd=fopen("d.out","w");
   outG=fopen("G.out","w");
   outw=fopen("W.out","w");
   }
...
fprintf(stderr,"HERE1, l=%d\n",l);
  if(isoflag ==6)
    {
    for(i=0; i < l; i++) /*Write weight file*/
      fprintf(outw,"%f\n",W[i]);
...
fprintf(stderr,"HERE2\n");
    ...
    if(isoflag==6)
      {
      for(i=0; i < cnt3; i++)  /*Write G matrix*/
         fprintf(outG,"%g %g %g %g %g %g\n",AJ[0][i],AJ[1][i],AJ[2][i],AJ[3][i],AJ[4][i],AJ[5][i]);
      fclose(outG);
      }
