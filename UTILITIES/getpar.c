...
register char *name, *value;	/* if necessary */
   {
	...
	if(ARGBUF == NULL || ARGLIST == NULL)
		gp_getpar_err("setpar","cannot allocate memory",0,0,0,0);
		...
		case 'v': switch(type[1])
			   {
				...
				default : typemess= "unknow vectorn (error)";
					break;
                          ...
                          return(0);
			  break;
		default : typemess= "unknown (error)"; return(0); break;
	   }
...
int gp_getpar_err(subname,mess,a1,a2,a3,a4)
char *subname, *mess;
int a1, a2, a3, a4;
   {
	fprintf(stderr,"\n***** ERROR in %s[%s] *****\n\t",
...
int *val;
   {
	...
	if(limit <= 0)
		gp_getpar_err("getpar","bad limit=%d specified",limit,0,0,0);
	index= 0;
	p= list;
		...
		switch(type[1])
		   {
			...
			default:
				gp_getpar_err("getpar",
					"bad vector type=%c specified",type[1],0,0,0);
				break;
		   }
		if(*p != '\0') p++;
	   }
